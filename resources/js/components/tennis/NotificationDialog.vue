...
const loadUpcomingLessons = async () => {
  ...
  try {
    ...
    const now = new Date()
    upcomingLessons.value = response.data.data
      .filter(lesson => {
      ...
      .map(lesson => ({
        id: lesson.id,
        display: `${formatDate(lesson.lesson_date)} - ${lesson.start_time}`,
        ...lesson
      }))
...
TC Zutendaal`
    ...
    case 'lesson_cancelled':
      notification.value.subject = 'Les geannuleerd - {date}'
      notification.value.message = `Beste {name},
...
TC Zutendaal`
    ...
    case 'schedule_change':
      notification.value.subject = 'Rooster wijziging - {group}'
      notification.value.message = `Beste {name},

Er is een wijziging in het lesrooster voor groep {group}.

[Beschrijf hier de wijziging]

Bekijk het nieuwe rooster op onze website of neem contact op bij vragen.
